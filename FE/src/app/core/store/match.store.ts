...
export class MatchStore {
  ...
  readonly currentMmr = computed(() => this.state()?.currentMmr ?? null);
  readonly currentTier = computed(() => this.state()?.currentTier ?? null);
  readonly rankUpTriggered = computed(() => this.state()?.rankUpTriggered ?? false);
  ...
  private computeDamageEvents(
    ...
    if (!oldState || !newState || !oldState.self || !newState.self || !oldState.opponent || !newState.opponent) return [];
    const now = Date.now();
