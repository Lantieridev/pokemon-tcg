...
export class LobbyComponent {
  ...
  async startMatch() {
    ...
    try {
      ...
      const res = await this.matchBackendService.createMatch('AshRivero', 'GarryBot', 1, 1).toPromise();
      const matchId = res!.matchId;
      
      this.wsService.connect(matchId).subscribe(state => {
        this.matchStore.updateState(state);
      });

      const initialState = await this.matchBackendService.getMatchState(matchId).toPromise();
      this.matchStore.updateState(initialState);
